...
const UploadQuicks: React.FC = () => {
    ...
    const handleFileDrop = useCallback((acceptedFiles: File[]) => {
        // Ensure only a single file is set for `file` (image or video)
    ...
    const handleUpload = () => {
        if (!description || !file || selectedFiles.length === 0) {
            alert("Please fill in all required fields: description, file, and images.");
            return;
        }
    ...
    return (
        <div className="w-full h-[80vh] flex items-center justify-center gap-5 flex-col mt-[40px]">
            ...
            <div
                {...getFileRootProps()}
                className="w-[30%] max-[650px]:w-full h-[60px] border-2 border-dashed border-blue-500 flex items-center justify-center cursor-pointer mb-4"
            >
                <input {...getFileInputProps()} />
                {isFileDragActive ? (
                    <p className="text-blue-50 text-center0">Drop the file here...</p>
                ) : (
            ...
            <div
                {...getImagesRootProps()}
                className="w-[30%] max-[650px]:w-full  h-[60px] border-2 border-dashed border-[#FFC300] flex items-center justify-center cursor-pointer"
            >
