...
import { PiNotebookLight, PiTicket, PiUsersThree } from "react-icons/pi";
import {
  IoVideocamOutline,
  IoQrCodeOutline,
  IoBagHandleOutline,
      ...
      <div className="w-full flex flex-col space-y-2 dark:text-white mt-4 p-2">
        <button
          className={`flex items-center px-4 py-3 w-full transition-colors ${
            isActive("/dashboard")
              ? "bg-black text-white border-l-4 border-[#FFC300]"
              : "text-black"
          }`}
        ...
        <button
          className={`flex items-center px-4 py-2 w-full transition-colors ${
            isActive("/dashboard/course/create")
              ? "bg-black text-white border-l-4 border-[#FFC300]"
              : "text-black"
          }`}
        ...
        <button
          className={`flex items-center px-4 py-2 w-full transition-colors ${
            isActive("/dashboard/ebook/create")
              ? "bg-black text-white border-l-4 border-[#FFC300]"
              : "text-black"
          }`}
        ...
        <button
          className={`flex items-center px-4 py-2 w-full transition-colors ${
            isActive("/dashboard/ticket")
              ? "bg-black text-white border-l-4 border-[#FFC300]"
              : "text-black"
          }`}
          onClick={() => handleNavigate("/dashboard/ticket")}
        >
          <PiTicket className="w-5 h-5 mr-3" />
